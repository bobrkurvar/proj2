...
@router.patch(
    ...
    responses={
        ...
        status.HTTP_204_NO_CONTENT: {"detail": "Пустое тело ответа"},
    },
...
async def delete_task_by_id(todo_id: int, manager: dbManagerDep):
    todo = await manager.delete(domain_model=Todo, ident_val=todo_id)
    return todo
...
async def delete_task_by_criteria_or_all(manager: dbManagerDep, ident: str | None = None, ident_val = None, ):
    await manager.delete(Todo, ident, ident_val)
