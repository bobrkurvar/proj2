...
from bot.utils.funcs import safety_delete_message, to_date_dict
from bot.utils.keyboards import get_inline_kb
from shared.celery_tasks import task1
...
async def process_fill_task_name(message: Message, state: FSMContext):
    ...
    msg = (
        await message.answer(text=phrases.fill_todo_content, reply_markup=kb)
    ).message_id
...
async def process_fill_task_content(message: Message, state: FSMContext):
    ...
    await safety_delete_message(message.bot, message.chat.id, msg)
    msg = (
        await message.answer(text=phrases.fill_todo_deadline, reply_markup=kb)
    ).message_id
...
async def process_fill_task_deadline_success(
    ...
    task1.apply_async(
        args=(message.chat.id, todo.get("name"), todo.get("id")), countdown=5
    )
    await safety_delete_message(message.bot, message.chat.id, msg)
...
async def process_fill_task_deadline_fail(message: Message, state: FSMContext):
    ...
    await safety_delete_message(message.bot, message.chat.id, msg)
    msg = (
        await message.answer(text=phrases.fail_fill_deadline, reply_markup=kb)
    ).message_id
    await state.update_data(msg=msg)


@router.callback_query(
    CallbackFactoryTodo.filter(), StateFilter(FSMTodoEdit.select_crit)
)
async def handle_param_button(
    ...
    msg = (
        await callback.message.edit_text(
            text=f"<b>ВВЕДИТЕ НОВОЕ {data_of_edit[callback_data.act.lower()]}</b>\n\n"
        )
    ).message_id
    await state.update_data(
        msg=msg, updating_data=callback_data.act.lower(), offset=callback_data.offset
    )
...
async def process_edit_todo(
    ...
    updating_data_content = (
        to_date_dict(message.text) if updating_data_name == "deadline" else message.text
    )
...
@router.callback_query(
    StateFilter(FSMSearch.filter),
    CallbackFactoryTodo.filter(F.act.lower().in_({"name", "content", "deadline"})),
)
async def handle_select_param(
    callback: CallbackQuery,
    callback_data: CallbackFactoryTodo,
    state: FSMContext,
    ext_api_manager: MyExternalApiForBot,
):
    lst = await ext_api_manager.read(
        prefix="todo",
        ident="doer_id",
        ident_val=callback.from_user.id,
        order_by=callback_data.act.lower(),
    )
    data = await state.get_data()
    if lst:
        text = " "
