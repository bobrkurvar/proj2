...
class InCachePageMiddleware(BaseMiddleware):
    ...
    async def __call__(
        ...
        state, callback_data, ext_api_manager = (
            data.get("state"),
            data.get("callback_data"),
            data.get("ext_api_manager"),
        )
        ...
        if pages is None:
            ...
            pages = {}
            to_update = await ext_api_manager.read(
                prefix="todo", doer_id=event.from_user.id, limit=limit, offset=offset
            )
